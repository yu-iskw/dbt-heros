...
- **Constraint**: `delete_member` MUST ONLY be used when `LIGHTDASH_TOOLS_SAFETY_MODE` is set to `write-destructive`.

## Behavior

1. **Safety Check**: Before attempting to use `delete_member`, check the environment variable `LIGHTDASH_TOOLS_SAFETY_MODE`. If it is not `write-destructive`, inform the user that the action is blocked.
