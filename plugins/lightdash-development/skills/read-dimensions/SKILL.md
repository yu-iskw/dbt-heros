# Read Dimensions

Skill for discovering and inspecting dimensions in the Lightdash semantic layer.

## Purpose

Enables identification and inspection of existing dimensions for modeling and analysis purposes.

## Tools

Wraps the following MCP tools from the `lightdash` server:

- `lightdash_tools__list_dimensions`

## Behavior

1. **Discovery**: Use `list_dimensions` to find available dimensions for a specific explore.

## Rules

- Use `read-dimensions` to understand current modeling before proposing changes.
